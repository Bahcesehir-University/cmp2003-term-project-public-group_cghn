...
void processLine(const string& line, unordered_map<string, long long>& zoneCounts, unordered_map<string, long long>& slotCounts) {
    if (line.empty()) return;

    size_t c1 = line.find(','); if (c1 == string::npos) return;
    size_t c2 = line.find(',', c1 + 1); if (c2 == string::npos) return;
    size_t c3 = line.find(',', c2 + 1); if (c3 == string::npos) return;
    size_t c4 = line.find(',', c3 + 1); if (c4 == string::npos) return;
    size_t c5 = line.find(',', c4 + 1); if (c5 == string::npos) return;

    string zoneId = line.substr(c1 + 1, c2 - c1 - 1);
    if (zoneId.empty()) return;

    string pickupTime = line.substr(c3 + 1, c4 - c3 - 1);
    if (pickupTime.length() < 13) return;


    char h0 = pickupTime[11], h1 = pickupTime[12];
    if (!isdigit((unsigned char)h0) || !isdigit((unsigned char)h1)) return;
...
void TripAnalyzer::ingestFile(string csvPath) {
    ...
    if (getline(file, line)) {
        if (line.find("vendor_id") == string::npos) { 
            processLine(line, zoneCounts, slotCounts);
        }
    }
